<?php
/*
Plugin Name: WordPress Popular Posts (Exp-Custom)
...
if ( !class_exists('WordpressPopularPosts') ) {
	...
	class WordpressPopularPosts extends WP_Widget {
		...
		private function __get_popular_posts( $instance ) {
			...
			//}
			foreach($mostpopular as $v => $p) {
				$count = $v + 1;
				$content .= $this->__render_popular_post( $p, $instance, $count);
			}
		 ...
		 * @return	string
		 */
		//add $count variable in function
		private function __render_popular_post($p, $instance, $count) {
			...
			// lines are added for getting an image  
			// start of getting image
			global $wpdb;
			$_im_default_image = get_post_meta($p->id,'_im_default_image', true );			
			$sql = "SELECT * FROM `wp_posts` WHERE id = " . $_im_default_image;
			$result = $wpdb->get_results($sql);		    
            $thumb = '<img src="'.$result[0]->guid.'"  title="'.$p->title.'" class="recent-posts-thumb">';
			...
			// build regular layout
				...
				$content =
					...
					. $thumb
					. '<span class="number_count">'.$count.'</span>'
