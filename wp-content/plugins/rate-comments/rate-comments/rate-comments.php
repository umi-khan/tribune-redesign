...
function commentsvote_showlink() {
    ...
    $p_id = get_the_ID();
    global $wpdb;
    $item_info = $wpdb->get_row('SELECT `likes` FROM `wp_comments_likes` WHERE comment_id = '.$current_CommentID.'
													AND post_id = '.$p_id.';');
 
// individual columns in the row may now be accessed like this:
	echo $item_info->column_name;
...
function commentsvote_ajaxhandler() {
	...
	if ( !$votecount ) {
	    ...
	    $results.='<div class="votescore comments-like"  ><a class="disabled number">'.'Recommend'.'</a>'.$votecountNew.'</div>'; 
