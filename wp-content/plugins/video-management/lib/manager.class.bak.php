...
class VM_manager
{
	...
	public function add($video_url)
	{
      ...
      if (strpos($video_url, 'youtube.com') > 0) {
         ...
         $youtube_helper = new Youtube_helper();

         $video_details = @array_pop( $youtube_helper->search_by_video_url( $video_url ) );

         if( false == $video_details ) return 'Please provide a valid youtube video url';

         if( $video_details instanceof Youtube_error ) return $video_details->message;
                  
         $url         = $video_details->url;
         $embed_url   = $video_details->player;
         $thumb       = $video_details->thumbnails['default'];
                  
         $title       = esc_html( $video_details->title );
         $caption     = '';
         $description = esc_html( $video_details->decription );
      
      } elseif (strpos($video_url, 'vimeo.com') > 0) {                 
         
         $error_message = 'Please provide a valid vimeo video url';
         $video_id = (int) substr(parse_url($video_url, PHP_URL_PATH), 1);
         if($video_id == 0 ) return 'Error 1: ' . $error_message;
         $video_raw_details = @file_get_contents("http://vimeo.com/api/v2/video/$video_id.php");
         if(empty( $video_raw_details) ) return 'Error 2: ' . $error_message;
         
         $video_details_array = unserialize($video_raw_details);
         if(empty( $video_details_array) ) return 'Error 3: ' . $error_message;
         
         $video_details = $video_details_array[0];
         if(empty( $video_details ) ) return 'Error 4: ' . $error_message;
      ...
      } elseif (strpos($video_url, 'dailymotion.com') > 0) {    
         
         $error_message = 'Please provide a valid Dailymotion video url';
         $video_id = basename($video_url);
         if(strlen($video_id) == 0 ) return $error_message . ' + 1';     

         $video_raw_details = @file_get_contents("https://api.dailymotion.com/video/$video_id?fields=thumbnail_url,title,description,embed_url");
         if(empty( $video_raw_details) ) return $error_message . ' + 2';
         
         $video_details =  json_decode($video_raw_details, TRUE);
         if(empty( $video_details) ) return $error_message . ' + 3';
      ...
      } else return 'Please provide a valid youtube or vimeo video url';
