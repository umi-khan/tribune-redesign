...
function exp_template_file($template, $data = null)
{
	...
	if( is_page() )
	{
		...
		$file = EXP_TEMPLATES_PATH . "template-parts/layouts/{$page->post_name}/{$page->post_name}-{$template}.php";
		if( file_exists( $file ) ) return include( $file );
	}
...
if (!function_exists('exp_insert_script'))
{
	function exp_insert_script($type = false)
	{
		static $script_queue = array('main'=>'main');	//default script if no script is loaded on the page and queue is empty
		if($type != false)
		{
			$script_queue[$type] = $type;
		}
		else
		{
			global $version;
			$scripts = implode(',',$script_queue);
			$script_src = get_template_directory_uri() . '/js/js.php?type=' . $scripts . '&amp;v='.$version;
			// use labjs only if non-blocking plugin is activated and labjs is added on the page
			if( class_exists('Nblocking_js') && wp_script_is(Nblocking_js::LABJS_HANDLE) )
			{
				echo '
			...
			{
				echo '<script src="' . $script_src .'" type="text/javascript" language="javascript" defer></script>';
			}
		}
	}
}

if (!function_exists('exp_insert_css'))
{
	function exp_insert_css($type=false, $media='screen')
	{
		global $version;
		
		if($media=='screen')
		{
			static $style_queue = array('main'=>'main');	//default style if no style is loaded on the page and queue is empty
			if($type != FALSE)
			{
				$style_queue[$type] = $type;
			}
			else
			{
				$styles = implode(',',$style_queue);
				$css_link = '<link rel="stylesheet" type="text/css" media="'.$media.'" href="' . get_template_directory_uri() . '/css/css.php?type=' . $styles . '&amp;v='.$version.'" />';
				return $css_link;
			}
		}
		else
		{
			echo '<link rel="stylesheet" type="text/css" media="'.$media.'" href="' . get_template_directory_uri() . '/css/css.php?type=' . $type . '&amp;v='.$version.'" />';
		}
	}
}
